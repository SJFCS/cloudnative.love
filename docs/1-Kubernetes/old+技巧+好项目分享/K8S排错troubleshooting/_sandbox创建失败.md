---
title: k8s中pod sandbox创建失败"failed to start sandbox container"
---

版权声明：本文为CSDN博主「qingwave」的原创文章，遵循CC 4.0 BY-SA版权协议，转载请附上原文出处链接及本声明。
原文链接：https://blog.csdn.net/u012986012/article/details/105291963/

## 背景

今天在k8s更新服务时,发现pod启动失败,报错`failed to start sandbox container`,如下所示:
...
首先,上节点看kubelet,docker有无异常,日志没有明显错误,通过`top`看到docker cpu占用非常高
...
按照以前的经验,一般是由某些容器引起的,通过`top`看到个别`sh`进程占用cpu较高.
