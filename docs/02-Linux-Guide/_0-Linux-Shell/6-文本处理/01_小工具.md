...
---
```
 head -n1 | awk '{printf $4}')"
 
 awk sed grep sort cut uniq cat echo fmt tr nl wc 

strace dtrace systemtap uname df history du lsb_release
```

进程：top htop ps atop lsof
系统性能：nmon iostat sar vmstat 
网络：traceroute mtr ping nmap netstat ss ufw/firewalld dig nslookup scp

grep 之类的工具再流水线中退出状态码不为0
...
grep -c \"restrict $master_hostip nomodify notrap noquery\" /etc/ntp.conf
```
