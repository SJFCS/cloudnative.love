LVM（Logical Volume Manager）是一种磁盘管理工具，它允许你将多个物理磁盘组合成一个逻辑卷，并且可以动态地对逻辑卷进行创建、删除、扩容、缩容等操作。在LVM中，有三个核心概念：PV（Physical Volume）、VG（Volume Group）和LV（Logical Volume）。

- PV：物理卷，是指物理磁盘或分区，用于存储数据。

- VG：卷组，是由一个或多个PV组成的逻辑卷集合，可以看做是一个虚拟磁盘。

- LV：逻辑卷，是VG上的一个逻辑分区，可以看做是一个虚拟分区。
